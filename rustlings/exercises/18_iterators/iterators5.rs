...
fn count_iterator(map: &HashMap<String, Progress>, value: Progress) -> usize {
    ...
    let mut len: usize = 0;
    let _: Vec<(&String, &Progress)> = map.into_iter().filter(|s| s.1 == &value).inspect(|_| len+= 1).collect();
    len
}
...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    let _: Vec<&HashMap<String, Progress>> = collection.into_iter().inspect(|map| count += count_iterator(map, value)).collect();
    count
}
